...
piece_final_keys AS (
    SELECT
        ...
        CASE
            WHEN p.jsonb->>'holdingId' IS NOT NULL THEN -- location id is nullable if no piece with same holding has it populated
...
-- Select the data that would be changed
SELECT
    prev.pol_id AS "PO Line ID",
    prev.current_locations_array AS "Old Locations",
    prev.new_locations_array AS "New Locations"
FROM po_lines_for_update prev
WHERE
    NOT (prev.current_locations_array @> prev.new_locations_array
         AND prev.current_locations_array <@ prev.new_locations_array);